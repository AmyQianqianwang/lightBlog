...
<script type="text/javascript">
      ...
      columns = [
          {title:'ID',dataIndex:'id',width:80,renderer:function(v){
            return Search.createLink({
              ...
              title : '友链信息',
              text : v,
              href : '${webapp.context}admin/link/detail.html?id=' + v
            });
          }},
          {title:'UID',dataIndex:'uid',width:100},
          {title:'名称',dataIndex:'name',width:200},
          {title:'网址',dataIndex:'url',width:300},
          ...
          {title:'操作',dataIndex:'',width:150,renderer : function(value,obj){
            var editStr =  Search.createLink({ //链接使用 此方式
                ...
                title : '编辑友链',
                text : '编辑',
                href : '${webapp.context}admin/link/edit.html?id=' + obj.id
              }),
              delStr = '<span class="grid-command btn-del" title="删除友链">删除</span>';//页面操作不需要使用Search.createLink
            return editStr + delStr;
          }}
        ],
      store = Search.createStore('${webapp.context}admin/link/page.json'),
    ...
    function editDialog() {
    	...
    	if(seles && seles.length == 1) {
            top.topManager.openPage({
                id : 'editLink' + seles[0].id,
                href : '${webapp.context}admin/link/edit.html?id=' + seles[0].id,
                title : '编辑友链'
              });
    	}else {
    ...
    function delItems(items){
      ...
      if(ids){
        BUI.Message.Confirm('确认要删除选中的记录么？',function(){
          $.ajax({
            url : '${webapp.context}admin/link/delete.html',
            dataType : 'json',
            data : ids,
  ...
  function addDialog() {
	  top.topManager.openPage({
	    id : 'addLink',
	    href : '${webapp.context}admin/link/add.html',
	    title : '新建友链'
	  });
  };
</script>
