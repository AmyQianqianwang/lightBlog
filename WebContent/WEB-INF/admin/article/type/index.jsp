...
<script type="text/javascript">
      ...
      columns = [
          {title:'ID',dataIndex:'id',width:80,renderer:function(v){
            return Search.createLink({
              id : 'articleTypeDetail' + v,
              title : '文章分类信息',
              text : v,
              href : '${webapp.context}admin/article/type/detail.html?id=' + v
            });
          }},
          {title:'名称',dataIndex:'name',width:400},
          ...
          {title:'操作',dataIndex:'',width:150,renderer : function(value,obj){
            var editStr =  Search.createLink({ //链接使用 此方式
                id : 'editArticleType' + obj.id,
                title : '编辑文章分类',
                text : '编辑',
                href : '${webapp.context}admin/article/type/edit.html?id=' + obj.id
              }),
              delStr = '<span class="grid-command btn-del" title="删除文章">删除</span>';//页面操作不需要使用Search.createLink
            return editStr + delStr;
          }}
        ],
      store = Search.createStore('${webapp.context}admin/article/type/page.json'),
    ...
    function editDialog() {
    	...
    	if(seles && seles.length == 1) {
            top.topManager.openPage({
                id : 'editArticleType' + seles[0].id,
                href : '${webapp.context}admin/article/type/edit.html?id=' + seles[0].id,
                title : '编辑文章分类'
              });
    	}else {
    ...
    function delItems(items){
      ...
      if(ids){
        BUI.Message.Confirm('确认要删除选中的记录么？',function(){
          $.ajax({
            url : '${webapp.context}admin/article/type/delete.html',
            dataType : 'json',
            data : ids,
  ...
  function addDialog() {
	  top.topManager.openPage({
	    id : 'addArticleType',
	    href : '${webapp.context}admin/article/type/add.html',
	    title : '新建文章分类'
	  });
  };
</script>
